...
@export var movement_speed: float = 3.00
...
func _interact() -> void:
		...
		if target.is_in_group("Station"):
			if hold_socket.item_stored and not target.item_stored:
				hold_socket.drop_item("Station")
			elif hold_socket.item_stored == false and target.is_cooking == false:
				hold_socket.grab_item(target.stored_item_id, "Station")
		
		if target.is_in_group("Table"):
			if hold_socket.item_stored and not target.item_stored:
				hold_socket.drop_item("Table")
			elif not hold_socket.item_stored and target.item_stored:
