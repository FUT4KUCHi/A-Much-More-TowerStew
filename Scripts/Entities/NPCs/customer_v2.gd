...
func _physics_process(delta):
	...
	if nav_agent.is_navigation_finished() and State.LEAVING and global_position.distance_to(entry_point.global_position) < 2:
		queue_free()
...
func check_arrival() -> void:
	if current_state == State.WAITING or current_state == State.IMPATIENT:
		check_order_item()
		
	if current_state == State.FIND_CHAIR and chair_position != Vector3.ZERO:
	...
	elif current_state == State.LEAVING and entry_point:
